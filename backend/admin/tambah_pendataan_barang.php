...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $namaFoto  = ''; // kolom foto NOT NULL -> pakai string kosong, bukan NULL
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            $namaFoto = 'brg_' . time() . '_' . rand(100, 999) . '.' . $ext;

            if (!move_uploaded_file($_FILES['foto']['tmp_name'], 'img/' . $namaFoto)) {
                $error    = 'Gagal memindahkan file foto ke folder img/.';
                $namaFoto = '';
            }
        }
    } elseif (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_INI_SIZE) {
    ...
    if ($error === '' && $nama !== '') {
        $stmt = mysqli_prepare(
            $conn,
            "INSERT INTO tb_barang (nama_barang, tgl, harga_awal, deskripsi_barang, foto)
             VALUES (?, ?, ?, ?, ?)"
        );
        mysqli_stmt_bind_param($stmt, 'ssiss', $nama, $tgl, $harga, $deskripsi, $namaFoto);

        if (mysqli_stmt_execute($stmt)) {
            header("Location: pendataan_barang.php?status=tambah_sukses");
            exit;
        }
        $error = 'Gagal menyimpan data: ' . mysqli_error($conn);

        // Kalau insert gagal, hapus foto yang terlanjur ter-upload
        if ($namaFoto !== '' && file_exists('img/' . $namaFoto)) {
            unlink('img/' . $namaFoto);
        }
    } elseif ($error === '') {
...
<head>
    ...
    <title>Tambah Barang - E-Lelang</title>
...
<div class="main-content">
    <div class="bg-white p-3 px-4 rounded-4 shadow-sm mb-4 d-flex justify-content-between align-items-center">
        <div>
            <h5 class="mb-0 fw-bold text-dark">Tambah Barang Lelang</h5>
            <small class="text-muted">Lengkapi data barang beserta fotonya</small>
        </div>
    ...
    <div class="card card-custom p-4">
        ...
        <!-- enctype WAJIB ada, kalau tidak file tidak akan terkirim -->
            ...
            <div class="row g-4">
                ...
                <div class="col-md-8">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="tgl" class="form-control"
                                   value="<?= isset($_POST['tgl']) ? htmlspecialchars($_POST['tgl']) : date('Y-m-d'); ?>">
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="harga_awal" class="form-control" min="0" required
                                       value="<?= isset($_POST['harga_awal']) ? htmlspecialchars($_POST['harga_awal']) : ''; ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="deskripsi_barang" class="form-control" rows="4"><?= isset($_POST['deskripsi_barang']) ? htmlspecialchars($_POST['deskripsi_barang']) : ''; ?></textarea>
                    </div>
                </div>
            </div>
...
<script>
    document.getElementById('inputFoto').addEventListener('change', function () {
        ...
        if (this.files && this.files[0]) {
            ...
            box.innerHTML = '<i class="fas fa-image fa-2x"></i>';
        }
    });
</script>
</body>
